...
class TodoyuCalendarViewHelper {
	...
	public static function getCalendarTitle($mode, $dateStart, $dateEnd) {
		...
		switch($mode) {
			...
			case 'day':
				$title	= TodoyuTime::format($dateStart, 'DlongD2MlongY4');				
				break;

			case 'week':
				if( date('n', $dateStart) === date('n', $dateEnd) ) {
					$title = date('j.', $dateStart) . ' - ' . TodoyuTime::format($dateEnd, 'D2MlongY4');
				} else {
					$title = TodoyuTime::format($dateStart, 'MlongD2') . ' - ' . TodoyuTime::format($dateEnd, 'D2MlongY4');
				}				
				break;
